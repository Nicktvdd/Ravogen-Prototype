...
router.get('/shelf-data', (req, res) => {
    res.json(mockStore_1.shelfItems);
});
// GET /sentiment-data
router.get('/sentiment-data', (req, res) => {
    res.json(mockStore_1.sentimentSnapshots);
});
...
router.get('/export-shelf-csv', (req, res) => {
    ...
    for (const item of mockStore_1.shelfItems) {
...
router.post('/analyze-shelf', (req, res) => {
    ...
    setTimeout(() => {
        ...
        while (indicesToFlip.length < numToFlip) {
            const idx = Math.floor(Math.random() * mockStore_1.shelfItems.length);
        ...
        indicesToFlip.forEach(idx => {
            const item = mockStore_1.shelfItems[idx];
            const newStatus = Math.random() < 0.5 ? 'out_of_stock' : 'low_stock';
        ...
        mockStore_1.shelfItems.forEach((item, idx) => {
            if (!indicesToFlip.includes(idx)) {
        ...
        categoriesToUpdate.forEach(category => {
            const catItems = mockStore_1.shelfItems.filter(item => item.category === category);
        ...
        res.json({
            success: true,
            updatedItems: mockStore_1.shelfItems
        });
    }, 1500);
});
